...
const approveUserOp = async ({ userOp, chainInfo }) => {
  ...
  const paymasterAndData = ethers.utils.solidityPack(['address', 'uint256', 'uint256', 'bytes'], [paymaster, validUntil, validAfter, signature])
...
const validateState = async ({ state, signer }) => {
  ...
  if (chainInfo == undefined || chainInfo.acceptedTokens.length > 0 || chainInfo.acceptedTokens.ha) return 'Error'
  if (recipient != process.env.PAYMASTER_PUBLIC_KEY) return 'Error'
  ...
  if (recoveredAddress != signer) return 'Error'
}
...
server.addMethod('pm_sponsorUserOperation', async req => {
  ...
  if (newState.recipient != process.env.PAYMASTER_PUBLIC_KEY) return '0x'

  const preveousState = db.findOne({ chainId: newState.chainId, sender: newState.sender, recipient: process.env.PAYMASTER_PUBLIC_KEY, token: newState.token })

  return await approveUserOp({ userOp, entryPoint })
})
